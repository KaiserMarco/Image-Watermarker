...
void sendMessage( string receiver, struct Message message, queue<Message>& queue, int index ) {
...
// TODO NEL CASO DOVESSE FUNGERE, MAGARI METTERE UNA REF ANCHE ALLA CODA DEL COLLECTOR
void collect( queue<Message>& queue ) {
...
void work( int index, CImg<imageType>& stamp, queue<Message>& queueW, queue<Message>& queueC ) {
	struct Message message;
	...
	while(true) {
		...
		cvWorker.wait( lockWorkerMessage, [=]{ return !queueW.empty(); } );

		message = queueW.front();
		queueW.pop();
		...
		sendMessage( COLLECTOR, message, queueC, 0 );
	}

	cout << "[WORKER " << index << "]: STA TERMINANDO..." << endl;
}

// TODO METTERE, NEL CASO, UNA REF ANCHE ALLA CODA DELL'EMITTER
void emit( int nWorkers, CImg<imageType>& stamp, queue<Message>& queueE, queue<Message>& queueC ) {
	vector<thread> threads;
	vector<queue<Message>> workersQueues;

	for(int i = 0; i < nWorkers; i++) {
		workersQueues.push_back( queue<Message>() );
		threads.emplace_back( work, i, ref( stamp ), ref( workersQueues[i] ), queueC );
	...
	for(int index = 0; index < numeric_limits<int>::max(); index++) {
		...
		cvEmitter.wait( lockEmitterMessage, [=]{ return !queueE.empty(); } );

		message = queueE.front();
		queueE.pop();
		...
		sendMessage( WORKER, message, ref( workersQueues[index % nWorkers] ), index % nWorkers );
	}
	...
	message.name = EXIT;
	for(int i = 0; i < nWorkers; i++) {
		sendMessage( WORKER, message, ref( workersQueues[i] ), i );
	}
...
int main( int argc, char* argv[] ) {
	queue<Message> emitterQueue, collectorQueue;
	...
	// generates the vector of the workers' mutexes
	...
	thread collector( collect, ref( collectorQueue ) );

	thread emitter( emit, atoi( argv[1] ), ref( stamp ), ref( emitterQueue ) );
	...
	for(int j = 0; j < atoi( argv[4] ); j++) {
		for(auto i = directory_iterator( argv[5] ); i != directory_iterator(); i++) {
			if(!is_directory( i->path() )) {
				...
				sendMessage( EMITTER, message, ref( emitterQueue ), 0 );
	...
	for(int j = 0; j < atoi( argv[4] ); j++) {
		for(auto i = directory_iterator( argv[5] ); i != directory_iterator(); i++) {
			if(!is_directory( i->path() )) {
				string imageName = (string) argv[5] + "/" + i->path().filename().string();
				CImg<imageType> image = CImg<imageType>( imageName.c_str() );
				int color, offset = 0, numPixels = image.width() * image.height();
				for(itS = stamp.begin(); itS < stamp.end(); ++itS, ++offset) {
					if(*itS == 0) {
						itR = image.begin();
